...
export const StatsBar: React.FC<Props> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-3 sm:px-4 py-2 text-xs text-slate-600 dark:text-slate-400">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2.5 sm:gap-3">
        {/* Syllabus Selection Toggle Button Group - Horizontal Scrollable on Mobile */}
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 overflow-x-auto max-w-full shrink-0">
          <button
            onClick={() => onSyllabusModeChange('all')}
            className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold flex items-center gap-1 sm:gap-1.5 transition-all cursor-pointer whitespace-nowrap shrink-0 ${
              syllabusMode === 'all'
        ...
        {/* Node Status Filter Pills - Scrollable / Flex Wrap */}
        <div className="flex items-center gap-1.5 flex-wrap overflow-x-auto max-w-full pb-0.5 sm:pb-0">
          <button
            onClick={() => onFilterModeChange('all')}
            className={`px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-medium transition-colors cursor-pointer whitespace-nowrap ${
              filterMode === 'all'
          ...
          <button
            ...
            className={`px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-medium flex items-center gap-1 transition-colors cursor-pointer whitespace-nowrap ${
